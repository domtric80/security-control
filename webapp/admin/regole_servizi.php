...
require_permission("regole_servizi", "read");

if ($_SERVER["REQUEST_METHOD"] === "POST") {
    verify_csrf();
    $filter_dom = (int)post("domanda_id_filter");
    $filter_srv = (int)post("servizio_id_filter");
    $action = (string)post("action", "add_rule");

    if ($action === "delete_rule") {
        require_permission("regole_servizi", "delete");
        delete_regola_servizio((int)post("id"));
        flash("success", "Regola eliminata.");
    } elseif ($action === "update_rule") {
        require_permission("regole_servizi", "update");
        update_regola_servizio(
            (int)post("id"),
            (int)post("gruppo_id"),
            (int)post("domanda_id"),
        ...
        require_permission("regole_servizi", (int)post("gruppo_id") > 0 ? "update" : "create");
        save_regole_servizi_gruppo(
            (int)post("servizio_id"),
            (string)post("nome"),
        ...
        require_permission("regole_servizi", "delete");
        delete_regole_servizi_gruppo((int)post("gruppo_id"), (int)post("servizio_id"));
        flash("success", "Gruppo regole eliminato con le sue condizioni.");
    } else {
        require_permission("regole_servizi", "create");
        $dom_id = (int)post("domanda_id");
        $srv_id = (int)post("servizio_id");
        $gruppo_id = (int)post("gruppo_id");
        $val = trim((string)post("valore_atteso", "1"));
        if ($dom_id && $srv_id && $gruppo_id) {
            save_regola_servizio($dom_id, $val, $srv_id, "OR", $gruppo_id);
            flash("success", "Regola aggiunta al gruppo.");
        }
        $filter_srv = $srv_id ?: $filter_srv;
    }
    $qs = http_build_query(array_filter(["domanda_id" => $filter_dom, "servizio_id" => $filter_srv]));
    redirect(APP_BASE_URL . "/admin/regole_servizi.php" . ($qs ? "?$qs" : ""));
}

$filter_dom = isset($_GET["domanda_id"]) ? (int)$_GET["domanda_id"] : null;
$filter_srv = isset($_GET["servizio_id"]) ? (int)$_GET["servizio_id"] : null;
$filter_gruppo = trim((string)($_GET["gruppo"] ?? ""));

$domande = get_domande(true);
$servizi = get_servizi(true);
$selected_servizio = $filter_srv ? get_servizio($filter_srv) : null;
$gruppi = $selected_servizio ? get_regole_servizi_gruppi((int)$selected_servizio["id"]) : get_all_regole_servizi_gruppi($filter_gruppo);
$page_title = "Regole Servizi";
...
<div class="row g-4">
  <div class="col-lg-4">
    <div class="card shadow-sm">
      <div class="card-header fw-semibold">Filtra servizio</div>
      <div class="card-body">
        <form method="get" class="row g-2">
          <div class="col-12">
            <label class="form-label small">Servizio</label>
            <select class="form-select form-select-sm" name="servizio_id">
              <option value="">-- seleziona servizio --</option>
              <?php foreach ($servizi as $s): ?>
              <option value="<?= (int)$s["id"] ?>" <?= (int)$s["id"] === $filter_srv ? "selected" : "" ?>>
                #<?= (int)$s["id"] ?> - <?= h(short_text($s["servizio_elementare"], 80)) ?>
              </option>
              <?php endforeach; ?>
            </select>
          </div>
    ...
    <div class="card shadow-sm mt-3">
      ...
      <div class="card-body">
        <form method="post" class="row g-3">
          ...
          <input type="hidden" name="servizio_id" value="<?= (int)$selected_servizio["id"] ?>">
          <input type="hidden" name="servizio_id_filter" value="<?= (int)$selected_servizio["id"] ?>">
          <input type="hidden" name="domanda_id_filter" value="<?= (int)$filter_dom ?>">
          <div class="col-12"><label class="form-label">Nome gruppo</label><input class="form-control" name="nome" placeholder="Es. Cloud + business critical" required></div>
  ...
  <div class="col-lg-8">
    <?php if (!$selected_servizio): ?>
      <div class="alert alert-info">
        Sto mostrando <strong>tutti i gruppi di regole servizi</strong>. Seleziona un servizio a sinistra per creare nuovi gruppi o aggiungere condizioni.
      </div>
    <?php else: ?>
      <div class="alert alert-light border">
        <strong>#<?= (int)$selected_servizio["id"] ?></strong> - <?= h($selected_servizio["servizio_elementare"]) ?><br>
        Il servizio viene suggerito se <strong>almeno uno</strong> dei gruppi sotto risulta vero.
      </div>
    <?php endif; ?>

      <div class="accordion" id="accordionRegoleServizi">
      <?php foreach ($gruppi as $gruppo): ?>
        <?php $groupRules = get_regole_servizi_by_gruppo((int)$gruppo["id"]); ?>
        <?php $collapseId = 'srvGroup' . (int)$gruppo["id"]; ?>
        <div class="accordion-item mb-2 shadow-sm">
          <h2 class="accordion-header">
            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#<?= h($collapseId) ?>">
              ...
              <span class="text-muted small">
                <?php if (!$selected_servizio): ?>
                  <?= h(short_text($gruppo["servizio_elementare"] ?? '', 80)) ?> ·
                <?php endif; ?>
                <?= count($groupRules) ?> condizioni
              </span>
            </button>
          </h2>
          <div id="<?= h($collapseId) ?>" class="accordion-collapse collapse" data-bs-parent="#accordionRegoleServizi">
          <div class="accordion-body">
            <?php if (!$selected_servizio): ?>
              <div class="small text-muted mb-2">
                Servizio: <?= h(short_text($gruppo["servizio_elementare"] ?? '', 130)) ?>
              </div>
            <?php endif; ?>
            <form method="post" class="row g-2 align-items-end">
              ...
              <input type="hidden" name="servizio_id" value="<?= (int)$gruppo["servizio_id"] ?>">
              <input type="hidden" name="servizio_id_filter" value="<?= $selected_servizio ? (int)$selected_servizio["id"] : '' ?>">
              <input type="hidden" name="domanda_id_filter" value="<?= (int)$filter_dom ?>">
          ...
          <div class="table-responsive">
            <table class="table table-sm table-hover align-middle mb-0">
              ...
              <?php $updateFormId = "updSrvRule" . (int)$reg["id"]; ?>
              <tr>
                <td>
                ...
                <td>
                  <select class="form-select form-select-sm" form="<?= h($updateFormId) ?>" name="gruppo_id">
                    <?php foreach ($gruppi as $g): ?>
                      <?php if ((int)$g["servizio_id"] === (int)$gruppo["servizio_id"]): ?>
      ...
      <?php if (!$gruppi): ?><div class="text-muted">Nessun gruppo presente per questo servizio.</div><?php endif; ?>
  </div>
</div>
